...
class VcoreCoursePointsController extends GetxController {
  ...
  static const int _gpaCacheSchemaVersion = 2;
  ...
  Future<void> getDanhSachKieuTruong() async {
    ...
    try {
      ...
      if (available.isEmpty) {
        // Không có dữ liệu thì để giao diện hiển thị trạng thái rỗng.
        // Không hiện snackbar, đặc biệt khi người dùng bật xem môn ngoài CTĐT.
        kieuTruong.value = null;
        hocKy.value = null;
        diemTrungBinhHocKy.value = null;
        return;
  ...
  Future<void> changeHocKy(String? displayName) async {
    final selectedSemester = danhSachHocKy.firstWhereOrNull(
    ...
    hocKy.value = selectedSemester;

    // Xóa GPA kỳ cũ ngay để giao diện không hiển thị nhầm trong lúc tải.
    diemTrungBinhHocKy.value = null;

    await _loadOfficialSemesterGpa(selectedSemester.id);

    final coursesForAi = _deduplicateCourses(
      diemThiTheoHocKy.values.toList(),
    );

    await _saveOfficialDataToCache(coursesForAi);
  }
  ...
  Future<bool> _loadFromCache(Map<String, dynamic> cachedData) async {
    try {
      if (cachedData['schemaVersion'] != _gpaCacheSchemaVersion) {
        return false;
      }
      ...
      final gpa4HocKy =
          cachedData['gpaHe4HocKy']?.toString() ?? '0.00';
      final gpa4TichLuy =
          cachedData['gpaHe4TichLuy']?.toString() ??
              cachedData['gpaHe4']?.toString() ??
              '0.00';
      final gpa10HocKy =
          cachedData['gpaHe10HocKy']?.toString() ?? '0.00';
      final gpa10TichLuy =
          cachedData['gpaHe10TichLuy']?.toString() ??
              cachedData['gpaHe10']?.toString() ??
              '0.00';
      final tinChiHocKy =
          cachedData['tongTinChiHocKy']?.toString() ?? '0';
      final tinChiTichLuy =
          cachedData['tongTinChiTichLuy']?.toString() ??
              cachedData['tongTinChi']?.toString() ??
              '0';
      final tinChiTruotHocKy =
          cachedData['tongTinChiTruotHocKy']?.toString() ?? '0';
      final tinChiTruotTichLuy =
          cachedData['tongTinChiTruotTichLuy']?.toString() ?? '0';

      diemTrungBinhHocKy.value = DiemTrungBinhModel(
        diemTrungBinhHe4HocKy: gpa4HocKy,
        diemTrungBinhHe4TichLuyDenHocKyHienTai: gpa4TichLuy,
        diemTrungBinhHe10HocKy: gpa10HocKy,
        diemTrungBinhHe10TichLuyDenHocKyHienTai: gpa10TichLuy,
        tongSoTinChiTichLuyHocKy: tinChiHocKy,
        tongSoTinChiTichLuyTichLuyDenHocKyHienTai: tinChiTichLuy,
        tongSoTinChiTruotHocKy: tinChiTruotHocKy,
        tongSoTinChiTruotTichLuyDenHocKyHienTai: tinChiTruotTichLuy,
      );
  ...
  Future<void> _afterApplyFullResponse() async {
    ...
    // Giữ danh sách loại trùng cho giao diện và AI Radar.
    // Không dùng danh sách này để tự tính GPA chính thức.
    diemThiHocKy.value = deduplicatedList;

    await _loadOfficialSemesterGpa(hocKy.value?.id);
    await _saveOfficialDataToCache(deduplicatedList);
    await _loadAiAnalysisFromCacheOnly(deduplicatedList);
  }

  Future<void> _loadOfficialSemesterGpa(String? semesterId) async {
    final normalizedSemesterId = semesterId?.trim() ?? '';

    // Luôn xóa giá trị kỳ trước trước khi gọi API.
    diemTrungBinhHocKy.value = null;

    if (normalizedSemesterId.isEmpty) {
      return;
    }

    final source = kieuTruong.value ?? 'TruongChinh';

    try {
      final result = await ApiRepository().getDiemTrungBinhHocKy(
      ...
      if (result.isEmpty) {
        // Không có GPA thì view tự ẩn card tổng kết.
        return;
      }

      // Nguồn GPA chính thức. Không quy đổi lại từ điểm chữ/điểm hệ 10.
      diemTrungBinhHocKy.value = result.first;

      debugPrint(
        '[GPA_OFFICIAL] term=$normalizedSemesterId '
            'gpa4HocKy=${result.first.diemTrungBinhHe4HocKy} '
            'gpa4TichLuy='
            '${result.first.diemTrungBinhHe4TichLuyDenHocKyHienTai}',
      );
    } catch (e) {
      // Khi xem môn ngoài CTĐT, API GPA có thể không có dữ liệu.
      // Đây không phải lỗi giao diện: chỉ ẩn card GPA.
      diemTrungBinhHocKy.value = null;
      debugPrint(
        '[GPA_OFFICIAL_EMPTY] term=$normalizedSemesterId error=$e',
      );
    }
  }
  ...
  Future<void> toggleXemCaMonNgoaiCtdt() async {
    ...
    // Không giữ GPA của chế độ trước trong lúc tải lại dữ liệu.
    diemTrungBinhHocKy.value = null;
    _mobileFullCache.clear();
  ...
  Future<void> _saveOfficialDataToCache(
      List<DiemThiHocKyModel> deduplicatedList,
      ) async {
    try {
      final gpa = diemTrungBinhHocKy.value;

      // Không có GPA chính thức thì không ghi số 0 đè lên cache hợp lệ.
      if (gpa == null) {
        return;
      }

      final data = {
        'schemaVersion': _gpaCacheSchemaVersion,
        'selectedHocKyId': hocKy.value?.id ?? '',

        // Các key cũ được giữ để không phá dashboard/consumer hiện hữu.
        // Chúng luôn mang giá trị tích lũy chính thức từ API.
        'gpaHe4': double.tryParse(
          gpa?.diemTrungBinhHe4TichLuyDenHocKyHienTai ?? '0',
        ) ??
            0.0,
        'gpaHe10': double.tryParse(
          gpa?.diemTrungBinhHe10TichLuyDenHocKyHienTai ?? '0',
        ) ??
            0.0,
        'tongTinChi': int.tryParse(
          gpa?.tongSoTinChiTichLuyTichLuyDenHocKyHienTai ?? '0',
        ) ??
            0,

        'gpaHe4HocKy': gpa?.diemTrungBinhHe4HocKy ?? '0.00',
        'gpaHe4TichLuy':
        gpa?.diemTrungBinhHe4TichLuyDenHocKyHienTai ?? '0.00',
        'gpaHe10HocKy': gpa?.diemTrungBinhHe10HocKy ?? '0.00',
        'gpaHe10TichLuy':
        gpa?.diemTrungBinhHe10TichLuyDenHocKyHienTai ?? '0.00',
        'tongTinChiHocKy': gpa?.tongSoTinChiTichLuyHocKy ?? '0',
        'tongTinChiTichLuy':
        gpa?.tongSoTinChiTichLuyTichLuyDenHocKyHienTai ?? '0',
        'tongTinChiTruotHocKy': gpa?.tongSoTinChiTruotHocKy ?? '0',
        'tongTinChiTruotTichLuy':
        gpa?.tongSoTinChiTruotTichLuyDenHocKyHienTai ?? '0',
