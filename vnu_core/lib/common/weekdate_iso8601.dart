...
class Week8601 extends Equatable {
  const Week8601({
    required this.year,
  ...
  factory Week8601.fromDate(DateTime date) {
    ...
    if (woy == 0) {
      ...
      return Week8601.fromDate(DateTime(date.year - 1, 12, 28));
    }
    ...
    if (woy == 53 &&
      ...
      return Week8601(year: date.year + 1, weekNumber: 1);
    }

    return Week8601(year: date.year, weekNumber: woy);
  }

  /// Return the current week.
  factory Week8601.current() => Week8601.fromDate(DateTime.now());

  /// Return a week initialized from an ISO formatted string like "2020W01" or "2020-W01" (or even "2020-W1" or "2020W1").
  factory Week8601.fromISOString(String isoString) {
    ...
    return Week8601(year: year, weekNumber: weekNumber);
  }
  ...
  Week8601 addWeeks(int weeks) {
    return Week8601.fromDate(day(0).addWeeks(weeks));
  }

  /// Return the week that many number of weeks in the past.
  Week8601 subtractWeeks(int weeks) {
    return Week8601.fromDate(day(0).subtractWeeks(weeks));
  }
  ...
  Week8601 get next => addWeeks(1);

  /// Return the week before.
  Week8601 get previous => subtractWeeks(1);
